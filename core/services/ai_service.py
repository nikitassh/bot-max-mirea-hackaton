    ...
    try:
        resp = await client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Сформулируй суть обращения студента в одном коротком предложении для преподавателя.",
                },
                {"role": "user", "content": text},
            ],
            max_tokens=80,
            ...
            messages=[
                {
                    "role": "system",
                    "content": (
                        "Ты помощник преподавателя. На основе базы знаний сформулируй короткий конкретный ответ студенту. "
                        "Пиши только сам ответ, без вступлений и пояснений. "
